...
## Fetch app-controller and its dependencies

Like the rest of Kubernetes, app-controller has used
...
```sh
git clone https://github.com/kubernetes/app-controller.git
cd app-controller
```
...
**Prerequisite**: Since the app-controller uses `apps/v1` deployments, the Kubernetes cluster version should be greater than 1.9.

```sh
# assumes you have a working kubeconfig, not required if operating in-cluster
go build -o app-controller .
./app-controller -kubeconfig=$HOME/.kube/config
